...
function extractCandidatesFromJson(json, source, feedUrl) {
  ...
  if (!Array.isArray(payload)) throw new Error('JSON feed must contain an array');
  ...
  for (const item of payload) {
    ...
    let url;
    try {
      url = stripTracking(new URL(String(item?.url || ''), source.homepage || feedUrl).toString());
    } catch {
      continue;
    }
    ...
    const publishedAt = parseDateString(item?.created_at) || toIsoDate(item?.created_at);
    ...
    results.push({
      ...
      fetchedAt: new Date().toISOString(),
      dateSource: 'json-api:created_at',
      datePrecision: 'exact',
