...
function extractCandidatesFromJson(json, source, feedUrl) {
  const payload = JSON.parse(json);
  const items = Array.isArray(payload) ? payload : payload?.records;
  if (!Array.isArray(items)) throw new Error('JSON feed must contain an array');
  ...
  for (const item of items) {
    ...
    const rawTitle = source.jsonFeedFormat === 'novaya-chronological'
      ? cheerio.load(String(item?.title || ''), null, false).text()
      : item?.title;
    ...
    let url;
    try {
      const rawUrl = item?.url || (source.jsonFeedFormat === 'novaya-chronological' && item?.slug
        ? `/articles/${item.slug}`
        : '');
      url = stripTracking(new URL(String(rawUrl), source.homepage || feedUrl).toString());
    } catch {
      continue;
    }
    ...
    const rawDate = item?.created_at || item?.dateISO || item?.publishedAt || item?.date;
    const publishedAt = parseDateString(rawDate) || toIsoDate(rawDate);
    ...
    results.push({
      ...
      country: source.country || null,
      url,
      publishedAt: applyTimezoneCorrection(publishedAt, source),
      fetchedAt: new Date().toISOString(),
      dateSource: source.jsonFeedFormat === 'novaya-chronological'
        ? 'json-api:dateISO'
        : 'json-api:created_at',
      datePrecision: 'exact',
      datePolicy: source.datePolicy?.summary || 'JSON API timestamp'
    });
  }
...
async function fetchSource(source) {
  ...
  if (source.jsonFeedUrl) {
    try {
      const json = await fetchWithTimeout(source.jsonFeedUrl, DEFAULT_TIMEOUT_MS, {
        requireHtml: false,
        accept: 'application/json,*/*;q=0.8'
      });
      sourceItems.push(...extractCandidatesFromJson(json, source, source.jsonFeedUrl));
    } catch (error) {
      errors.push({ source: source.name, url: source.jsonFeedUrl, kind: 'json-feed', message: error.message });
    }
  }

  // Официальный JSON — самый быстрый и точный путь. RSS и HTML нужны только
  // как резерв, если API источника временно недоступен или вернул пустой список.
  const useLegacyFallback = !source.jsonFeedUrl || sourceItems.length === 0;

  if (useLegacyFallback && USE_FEEDS && Array.isArray(source.feedUrls)) {
  ...
  const stats = {
    ...
    jsonFeed: Boolean(source.jsonFeedUrl),
