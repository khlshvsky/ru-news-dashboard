...
//
//  Пароль в переменных окружения лежит хешем, не текстом:
//    AUTH_USERNAME       — логин
//    AUTH_PASSWORD_HASH  — pbkdf2$<итерации>$<соль_b64>$<хеш_b64>
//    AUTH_SECRET         — секрет для подписи сессионной куки (32+ байта)
//
//  Хеш и секрет генерируются через `node tools/hash-password.mjs`.
// ─────────────────────────────────────────────────────────────────────────────
...
export function getAuthConfig() {
  ...
  return { username, passwordHash, secret, missing, configured: missing.length === 0 };
}
